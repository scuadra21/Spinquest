...
for proc in ["Brem","Eta"]:
    ...
    for mass in masses[proc]:
        ifile = "simana_electrons_Apr22_500ev_%s/%s_%s_z500_600_eps_-6.4.root"%(proc, proc,mass)
        ...
        for m in masses:
            for ev in events:
                if ev.getMass() == m and ev.getEps() == eps:
                    try:
                        cross[i].append(ev.getRatio()*float(EMCAL_acc_dict[m])*new_rates['%.3f'%m]) #*POT*eps**2) # this is not needed anymore since its accounted for in the new_rates
                    except KeyError:
                        not_found+=1
                        cross[i].append(ev.getRatio()*new_rates['%.3f'%m])
